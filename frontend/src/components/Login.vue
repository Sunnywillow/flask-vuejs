        ...
        <div class="col-md-4">
          <form @submit.prevent="onSubmit">
            <div class="form-group">
              ...
              <input type="text" v-model="loginForm.username" class="form-control" v-blind:class="{'is-invalid': loginForm.usernameError}" id="username" placeholder="">
              <div v-show="loginForm.usernameError" class="invalid-feedback">{{ loginForm.usernameError }}</div>
            </div>
            <div class="form-group">
              <label for="password">Password</label>
              <input type="password" v-model="loginForm.password" class="form-control" v-blind:class="{'is-invalid': loginForm.password}" placeholder="">
    ...
    data () {
      return {
        ...
        loginForm: {
          username: '',
          password: '',
          submitted: false, // 是否点击了submit 按钮
    ...
    methods: {
      onSubmit (e) {
        ...
        if (this.loginForm.errors > 0) {
          // 表单验证没通过时， 不继续往下执行，即不会通过axios 调用后端API
          return false
        }

        const path = 'http://localhost:5000/api/tokens'
        // axios 实现Basic Auth需要在config中设置auth 这个属性即可
        axios.post(path, {}, {
          auth: {
